...
async function shouldGenerate(outputPath: string, sourcePaths: string[]): Promise<boolean> {
...
async function generateOGImage(
  ...
  const blurredBackground = await createBlurredBackground(heroImagePath);
...
async function main() {
  try {
    const fontRegular = await readFile('./node_modules/@fontsource/roboto/files/roboto-latin-400-normal.woff');
    const fontBold = await readFile('./node_modules/@fontsource/roboto/files/roboto-latin-700-normal.woff');
    ...
    for (const postPath of posts) {
      const fullPath = join(CONTENT_DIR, postPath);
      const slug = postPath.replace(/\/index\.md$/, '');
      ...
      try {
        ...
        const cleanHeroImage = String(data.heroImage).replace(/^\.\//, '');
        const heroImagePath = join(CONTENT_DIR, slug, cleanHeroImage);
        const outputPath = join(OUTPUT_DIR, `${slug.toLowerCase()}.png`);

        if (!(await shouldGenerate(outputPath, [fullPath, heroImagePath]))) {
          console.log(`Skipped OG image for: ${slug}`);
          continue;
        }

        await generateOGImage(String(data.title), slug, heroImagePath, fonts);
      } catch (error) {
